...
const CreateFolder = ({activeFolder, setFolders, setCreateFolder}) => {
	const createNewFolder = (e) => {
		...
		if(e.target.name.value.length > 0 && e.target.name.value !== '/'){
			...
			axios.post('http://79.137.204.172/api/storage/create-folder/', {parent: Number(activeFolder), name: folderName}, {headers: {'Authorization' : `Bearer ${localStorage.getItem('token')}`}})
			.then(response => {
