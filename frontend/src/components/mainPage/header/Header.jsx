...
function Header({changeTheme, modalOpen, profileClick, profilePhoto, setFiles, formatFileSize, activeFolder, folderModal}) {
    const token = localStorage.getItem('token')
    const navigate = useNavigate()
    const logoutClick = () => {
        axios.post('https://best-edu-server.ru/api/user/logout/', token, {headers: {'Authorization': `Bearer ${token}`}})
        .catch(error => {
    ...
    const filesPush = (e) => {
        ...
        axios.post('https://best-edu-server.ru/api/storage/upload-file-to-server/', formData, {headers: {'Authorization' : `Bearer ${token}`}})
        .then(response => {
    ...
    return( 
        <header>
            ...
            <div className="header_item2">
                <svg className='modal-opener' width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => folderModal()}>
                    <path className='theme1 modal-opener' d="M11.25 16.25H18.75M15 12.5V20M15.0784 7.57842L14.9216 7.42158C14.4892 6.98919 14.273 6.773 14.0208 6.6184C13.797 6.48133 13.5531 6.38031 13.2981 6.31908C13.0104 6.25 12.7046 6.25 12.0931 6.25H7.75C6.34987 6.25 5.6498 6.25 5.11503 6.52249C4.64461 6.76216 4.26216 7.14461 4.02249 7.61503C3.75 8.1498 3.75 8.84986 3.75 10.25V19.75C3.75 21.1501 3.75 21.8503 4.02249 22.385C4.26216 22.8554 4.64461 23.2379 5.11503 23.4775C5.6498 23.75 6.34986 23.75 7.75 23.75H22.25C23.6501 23.75 24.3503 23.75 24.885 23.4775C25.3554 23.2379 25.7379 22.8554 25.9775 22.385C26.25 21.8503 26.25 21.1501 26.25 19.75V12.75C26.25 11.3499 26.25 10.6498 25.9775 10.115C25.7379 9.64461 25.3554 9.26216 24.885 9.02249C24.3503 8.75 23.6501 8.75 22.25 8.75H17.9069C17.2954 8.75 16.9896 8.75 16.7019 8.68092C16.4469 8.61969 16.203 8.51867 15.9792 8.3816C15.727 8.227 15.5108 8.01081 15.0784 7.57842Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
