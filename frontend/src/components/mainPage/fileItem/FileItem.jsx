...
function FileItem({file, setFiles, setTrashFiles}) {
    const handleRemove = () => {
        ...
        axios.put('https://best-edu-server.ru/api/storage/move-file-to-trash/', {"id": file.id}, {headers: {"Authorization": `Bearer ${token}`}})
        .then(() => {
    ...
    const fileDownload = () => {
        const token = localStorage.getItem('token')
        axios.post('https://best-edu-server.ru/api/storage/download-file-from-server/', {"id": file.id}, {responseType: 'blob', headers: {'Authorization' : `Bearer ${token}`}})
        .then(response => {
        ...
        <div className="file-item">
            ...
            <div className="file-item_item">
                ...
                <div className="file-item_item2">
                    <a onClick={() => fileDownload()} download> 
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
                            <path className='theme5' d="M7.5 26.25H22.5M15 3.75V21.25M15 21.25L21.25 15M15 21.25L8.75 15" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                    </a>
                    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none" onClick={() => handleRemove()}>
