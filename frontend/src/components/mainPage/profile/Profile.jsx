...
function Profile ({profilePhoto, profileEmail, setProfileEmail, setProfilePhoto}) { 
    ...
    const sendProfileData = (e) => {
        ...
        axios.put('https://best-edu-server.ru/api/user/edit-email/', profileData, {headers:{'Content-Type': 'application/json', "Authorization": `Bearer ${token}`}})
        .then((response) => {
    ...
    const sendNewPassword = (e) => {
        ...
        axios.put('https://best-edu-server.ru/api/user/change-password/', newPasswrodData, {headers:{'Content-Type': 'application/json', "Authorization": `Bearer ${token}`}})
        .then(() => {
            document.querySelector('.send').style = `border: 2px solid lightgreen`
        })
        .catch((error) => {
    ...
    const photoUpload = (e) => {
        ...
        axios.put('https://best-edu-server.ru/api/user/edit-profile-photo/', formData, {headers: {'Authorization': `Bearer ${token}`}})
        .then(response => {
