...
const FolderItem = ({folder, getFolderData, setFolders, setTrashFolders, setIdStorage}) => {
	...
	const handleRemove = (event) => {
        ...
        axios.put('https://best-edu-server.ru/api/storage/move-folder-to-trash/', {"id": folder.id}, {headers: {"Authorization": `Bearer ${token}`}})
        .then(() => {
	...
	const sendFolderNewName = (e) => {
		...
		if(newName.length > 0 && newName !== '/'){
			axios.put('https://best-edu-server.ru/api/storage/rename-folder/', {id: folder.id, name: newName}, {headers: {'Authorization': `Bearer ${localStorage.getItem('token')}`}})
			.then(response => {
				setFolders(prev => {
